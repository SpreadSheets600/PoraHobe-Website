...
export async function syncNoteLinks(
  ...
  const existingLinks: any[] = (await db
    .prepare('SELECT url, watch_progress FROM links WHERE note_id = ?')
    .bind(noteId)
    .all()
  ).results || [];
  ...
  for (const item of parsed) {
    ...
    await db
      ...
      .bind(id, noteId, item.url, item.type, item.title, item.thumbnail_url, progress, now)
      .run();
  }
}
...
export async function syncBacklinks(
  ...
  const uuidRegex = /\/notes\/([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})/gi;
  ...
  await db.prepare('DELETE FROM backlinks WHERE source_note_id = ?').bind(noteId).run();
  
  // Insert new backlinks
  for (const targetId of targetIds) {
    if (targetId === noteId) continue;
    try {
      const exists = await db.prepare('SELECT id FROM notes WHERE id = ?').bind(targetId).first();
      if (exists) {
        await db
          .prepare('INSERT OR IGNORE INTO backlinks (source_note_id, target_note_id) VALUES (?, ?)')
